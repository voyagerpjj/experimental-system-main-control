...
static void communicat_runing()
{
	...
	if (rx_ttl_message_temp.state == UPDATA)
	{
		...
		else if (rx_ttl_message_temp.rx_message_state == CONFIGURATION_MSG)
		{
			...
			if (rx_ttl_message_temp.configuration_data.rx_modbus_message == DEVICE_POWER_MSG)	// 如果是电源配置指令
			{
				for (int i = 0; i < 4; i++)
				{
					liquid_level_transmitter_typedef liquid_level_transmitter_temp = communicat_get_liquid_level_transmitter(i);
