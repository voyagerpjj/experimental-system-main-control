...
// 静态函数声明
static void manager_usart_start_dma_receive(manager_usart_typedef *manager_usart);
...
void manager_usart_errorCallback(UART_HandleTypeDef *huart);
...
/**
  ...
  * @retval  void
  */
void ifr_uasrt_init(manager_usart_typedef *manager_usart, UART_HandleTypeDef *huart, void(*UART_Analysis_Function)(uint8_t *pData, uint8_t len)) 
{
	...
	if (UART_Analysis_Function != NULL)
			manager_usart->AnalysisFunc = UART_Analysis_Function;

	// 注册接收回调函数 和 错误回调函数
	HAL_UART_RegisterRxEventCallback(huart, manager_usart_rx_callback);
	HAL_UART_RegisterCallback(huart, HAL_UART_ERROR_CB_ID, manager_usart_errorCallback);
	
	// 启动DMA接收
	manager_usart_start_dma_receive(manager_usart);
}
...
{
	// 切换缓冲区并保存数据长度
	manager_usart->Buffer_Num = !manager_usart->Buffer_Num;
	manager_usart->Data_Length = len;
	
	// 如果设置了解析函数，则调用
	if (manager_usart->AnalysisFunc != NULL) 
			manager_usart->AnalysisFunc(manager_usart->rx_dma_buffer_[!manager_usart->Buffer_Num], len);
	
	// 重新启动DMA接收
	manager_usart_start_dma_receive(manager_usart);
	
	// 更新时间戳
	manager_usart->_updata_systick = HAL_GetTick();
}
...
/**
  ...
  * @retval  void
  */
static void manager_usart_start(manager_usart_typedef *manager_usart) 
{ 
	// 启动DMA空闲接收
	manager_usart->UsartRxState = HAL_UARTEx_ReceiveToIdle_DMA(manager_usart->_huart, manager_usart->rx_dma_buffer_[manager_usart->Buffer_Num], USART_RX_RING_BUFFER_SIZE);
...
static void manager_usart_restart(manager_usart_typedef *manager_usart) 
{
	...
	HAL_UART_RegisterCallback(manager_usart->_huart, HAL_UART_ERROR_CB_ID, manager_usart_errorCallback);
